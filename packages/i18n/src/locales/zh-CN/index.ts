...
export const zhCN = {
  ...
  tray: {
    ...
    pinned: "置顶",
    viewMore: "查看更多…",
    open: "打开 Pi-Desktop-Next",
    quit: "退出 Pi-Desktop-Next",
    askTitle: "关闭后继续在后台运行 Pi-Desktop-Next？",
    askBody:
      "关闭窗口后，Pi-Desktop-Next 可以继续在系统托盘中运行，避免丢失任何内容。你随时可以在设置中更改此选项。",
    closeToTray: "关闭到托盘",
    confirmQuitTitle: "确定退出 Pi-Desktop-Next？",
    confirmQuitBody:
  ...
  chat: {
    ...
    placeholder: "让 Pi-Desktop-Next 帮你做任何事",
...
sklm: {
    ...
    pluginProviderKeyHint: "保存在 Pi-Desktop-Next 中，由运行时使用。插件拿不到此密钥。",
  ...
  plugins: {
    ...
    permissionHelp: {
      ...
      "provider.register": "把此插件定义的服务添加到设置的服务列表。接口地址和模型由插件提供，API 密钥则留在 Pi-Desktop-Next 中。",
      "desktop.control":
        "允许插件调用经过审查的 Pi-Desktop-Next 操作目录；破坏性操作仍需 confirm=true，插件永远拿不到 MCP bearer token。",
      ...
      "keyboard.globalShortcut": "注册系统级快捷键，在 Pi-Desktop-Next 未聚焦时触发本插件自己的命令。",
  ...
  status: {
    ...
    dbSchemaTooNew:
      "当前 Pi-Desktop-Next 比本地数据更旧（数据库 schema {{found}}，此版本仅支持 {{supported}}）。请安装上次打开这些数据的更新版本，或更高版本。",
    archMismatch:
