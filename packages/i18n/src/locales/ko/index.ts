...
export const ko = {
  ...
  tray: {
    ...
    pinned: "고정됨",
    viewMore: "더 보기…",
    open: "Pi-Desktop-Next 열기",
    quit: "Pi-Desktop-Next 종료",
    askTitle: "Pi-Desktop-Next을 백그라운드에서 계속 실행할까요?",
    askBody:
      "창을 닫아도 Pi-Desktop-Next을 시스템 트레이에서 계속 실행하여 작업을 잃지 않을 수 있습니다. 이 설정은 언제든지 설정에서 변경할 수 있습니다.",
    closeToTray: "트레이로 닫기",
    confirmQuitTitle: "Pi-Desktop-Next을 종료할까요?",
    confirmQuitBody:
  ...
  chat: {
    ...
    placeholder: "Pi-Desktop-Next에 무엇이든 도움을 요청하세요",
...
sklm: {
    ...
    pluginProviderKeyHint: "Pi-Desktop-Next에 저장되고 런타임에서 사용됩니다. 플러그인은 이 키를 받지 않습니다.",
...
importConfirm: "가져온 확장은 에이전트 프로세스 안에서 에이전트 자체 도구와 같은 권한으로 실행됩니다. 선언된 스킬은 에이전트에 지침을 추가할 수 있고, 선언된 의존성은 npm으로 설치됩니다(설치 스크립트 비활성화). 계속할까요?",
    ...
    permissionHelp: {
      ...
      "provider.register":
        "이 플러그인이 정의한 프로바이더를 설정의 프로바이더 목록에 추가합니다. 플러그인은 엔드포인트와 모델을 제공하며, API 키는 Pi-Desktop-Next에 남습니다.",
      "desktop.control":
        "플러그인이 검토된 Pi-Desktop-Next 제어 카탈로그를 호출할 수 있습니다. 파괴적 작업에는 여전히 confirm=true가 필요하며 MCP bearer token은 노출되지 않습니다.",
      ...
      "keyboard.globalShortcut": "Pi-Desktop-Next이 포커스되지 않은 상태에서 이 플러그인 자체 명령을 실행하는 시스템 전역 키보드 단축키를 등록합니다.",
  ...
  status: {
    ...
    dbSchemaTooNew:
      "이 Pi-Desktop-Next은 로컬 데이터보다 오래된 버전입니다(데이터 스키마 {{found}}, 이 버전은 {{supported}}까지 지원). 이 데이터를 마지막으로 연 최신 Pi-Desktop-Next 또는 그 이후 버전을 설치하세요.",
    archMismatch:
