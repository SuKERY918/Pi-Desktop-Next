...
test("Windows and Linux use menu-free frameless chrome with window controls", () => {
  ...
  // The band must stay unpainted. It is position:fixed over a veiled surface,
  // so any veil of its own would compose with that surface (56% over 56% = 81%)
  // and show up as a hard side seam against the adjacent titlebar.
  assert.match(
    stylesSource,
    /\.window-controls\s*\{[^}]*height:\s*var\(--ds-toolbar-height\)[^}]*padding-left:\s*8px;[^}]*background:\s*transparent;/s,
  );
