#!/bin/bash
# Open a trusted unsigned Pi-Desktop-Next installation on macOS.
#
...
readonly APP_BUNDLE_NAME="Pi-Desktop-Next.app"
readonly EXPECTED_BUNDLE_ID="io.github.sukery918.pi-desktop-next"
...
if [[ -z "$app_path" ]]; then
  show_alert \
    "Pi-Desktop-Next is not installed" \
    "Drag Pi-Desktop-Next.app to Applications, then double-click this helper again."
  exit 1
fi
...
if [[ "$bundle_identifier" != "$EXPECTED_BUNDLE_ID" ]]; then
  show_alert \
    "This is not a Pi-Desktop-Next app" \
    "The helper only opens the official Pi-Desktop-Next bundle."
  exit 1
fi
...
if [[ "$quarantine_attributes" == *"com.apple.quarantine"* ]]; then
  if ! /usr/bin/xattr -r -d com.apple.quarantine "$app_path"; then
    show_alert \
      "Pi-Desktop-Next could not be opened" \
