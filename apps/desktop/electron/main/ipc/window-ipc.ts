...
import { usesNativeWindowMaterial } from "../bootstrap/window-material";
...
export function registerWindowIpc({
  ...
  handle(IPC.invoke.windowSetBackgroundColor, async (input: unknown = {}) => {
    ...
    /*
     * On Windows 11 22H2+ the window surface is the native acrylic material and
     * the plate is deliberately transparent. Painting an opaque background here
     * would cover that material, so the FIRST theme or plugin switch would turn
     * the window back into a solid slab. Skip the paint and report
     * `applied: false`; the renderer recomputes its own veils from the palette
     * and never reads this field for anything but the round-trip. Linux and
     * pre-22H2 Windows have no material, so they keep painting the opaque
     * background exactly as before.
     */
    if (usesNativeWindowMaterial()) return { applied: false, theme };
