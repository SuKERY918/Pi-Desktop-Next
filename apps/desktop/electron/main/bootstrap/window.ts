...
import { activateWindowMaterial, windowMaterialOptions } from "./window-material";
...
export async function createWindow({
  ...
  windowState.mainWindow = new BrowserWindow({
    ...
    //
    // On Windows 11 22H2+ the Windows/Linux branch asks DWM for the native
    // "acrylic" material (window-material.ts) and leaves the window plate
    // transparent so the blurred desktop shows through the renderer's veils.
    // Elsewhere — Linux and pre-22H2 Windows — it degrades to the opaque,
    // theme-matched `builtinWindowBackground(...)`, never a transparent window.
  ...
  // Declaring `backgroundMaterial` in the constructor is necessary but not
  // sufficient: Electron can drop it, and the first painted frame has been
  // reported to lose it (electron/electron#43345). Re-assert it once the
  // renderer has loaded; the call is idempotent and a no-op off Windows 11.
  window.webContents.on("did-finish-load", () => {
    activateWindowMaterial(window, "did-finish-load", logger);
  });
  ...
  window.once("ready-to-show", () => {
    if (!isLiveWindow()) return;
    // Re-assert the native material before the first paint is shown, so the
    // window never flashes its (transparent) plate without the acrylic behind
    // it. No-op on hosts without the material.
    activateWindowMaterial(window, "ready-to-show", logger);
